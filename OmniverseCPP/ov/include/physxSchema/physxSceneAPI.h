...
public:
    ...
    // BOUNCETHRESHOLD 
    // --------------------------------------------------------------------- //
    /// A contact with a relative velocity below this will not bounce. Allowed range [0, max_float). Default value 0.2 * toleranceSpeed.
    ///
    /// | ||
    /// | -- | -- |
...
public:
    ...
    // --------------------------------------------------------------------- //
    /// A threshold of contact separation distance used to decide if a contact point will experience friction forces. Allowed range [0, max_float). Default value 0.04 * tolerancesLength.
    ///
    /// | ||
    /// | -- | -- |
...
public:
    ...
    // --------------------------------------------------------------------- //
    /// Friction correlation distance used to decide whether contacts are close enough to be merged into a single friction anchor point or not. Allowed range [0, max_float). Default value 0.025 * tolerancesLength.
    ///
    /// | ||
    /// | -- | -- |
...
public:
    ...
    // MAXBIASCOEFFICIENT 
    // --------------------------------------------------------------------- //
    /// The maximum bias coefficient used in the constraint solver. Allowed range [0, inf)
    ///
    /// | ||
    /// | -- | -- |
...
public:
    ...
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->UInt |
    PHYSXSCHEMA_API
    UsdAttribute GetMinIterationCountAttr() const;

    /// See GetMinIterationCountAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    PHYSXSCHEMA_API
    UsdAttribute CreateMinIterationCountAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->UInt |
    PHYSXSCHEMA_API
    UsdAttribute GetMaxIterationCountAttr() const;

    /// See GetMaxIterationCountAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    PHYSXSCHEMA_API
    UsdAttribute CreateMaxIterationCountAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
