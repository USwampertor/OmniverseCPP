...
///
/// Applied to a PhysxParticleSystem. Defines settings that the particle system
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `int physxDiffuseParticles:maxDiffuseParticles = 100000` |
    /// | C++ Type | int |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Int |
    PHYSXSCHEMA_API
    UsdAttribute GetMaxDiffuseParticlesAttr() const;

    /// See GetMaxDiffuseParticlesAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    PHYSXSCHEMA_API
    UsdAttribute CreateMaxDiffuseParticlesAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
