...
///
class PhysxSchemaPhysxParticleSystem : public UsdTyped
{
public:
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float particleContactOffset = -inf` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    PHYSXSCHEMA_API
...
public:
    ...
    // SMOOTHING 
    // --------------------------------------------------------------------- //
    /// Smoothing coefficient for particle positions. Does not affect dynamics, only rendering output.
    /// Range: [0, 1]
    /// Units: dimensionless
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float smoothing = 0` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    PHYSXSCHEMA_API
...
public:
    ...
    // ENABLEANISOTROPY 
    // --------------------------------------------------------------------- //
    /// Enables use of fluid anisotropy information for rendering. Does not affect dynamics, only rendering output.
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `bool enableAnisotropy = 0` |
    /// | C++ Type | bool |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Bool |
    PHYSXSCHEMA_API
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float anisotropyScale = 1` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    PHYSXSCHEMA_API
    UsdAttribute GetAnisotropyScaleAttr() const;

    /// See GetAnisotropyScaleAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    PHYSXSCHEMA_API
    UsdAttribute CreateAnisotropyScaleAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float anisotropyMin = 0.2` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    PHYSXSCHEMA_API
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float anisotropyMax = 2` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    PHYSXSCHEMA_API
...
public:
    ...
    // MAXPARTICLES 
    // --------------------------------------------------------------------- //
    /// The maximum number of particles in the current particle system. The maximum is increased automatically when particle objects are added to the system.
    /// The user can only increase the maximum, decreasing is not allowed.
    /// Range: [max(0, maxParticles), inf)
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `int maxParticles = 0` |
    /// | C++ Type | int |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Int |
    PHYSXSCHEMA_API
    UsdAttribute GetMaxParticlesAttr() const;

    /// See GetMaxParticlesAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    PHYSXSCHEMA_API
    UsdAttribute CreateMaxParticlesAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
