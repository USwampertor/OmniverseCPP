...
//
#ifndef PHYSXSCHEMA_GENERATED_PLANE_H
#define PHYSXSCHEMA_GENERATED_PLANE_H
...
public:
    ...
    /// an invalid \p prim
    explicit PhysxSchemaPlane(const UsdPrim& prim=UsdPrim())
        : UsdGeomMesh(prim)
    {
    }
    ...
    explicit PhysxSchemaPlane(const UsdSchemaBase& schemaObj)
    ...
    /// Return a PhysxSchemaPlane holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// PhysxSchemaPlane(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    PHYSXSCHEMA_API
    static PhysxSchemaPlane
