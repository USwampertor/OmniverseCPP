...
public:
    ...
    // GRIDFILTERINGFLAGS 
    // --------------------------------------------------------------------- //
    /// Encoded grid filtering passes.
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `int physxIsosurface:gridFilteringFlags = 19528` |
    /// | C++ Type | int |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Int |
    PHYSXSCHEMA_API
    UsdAttribute GetGridFilteringFlagsAttr() const;

    /// See GetGridFilteringFlagsAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    PHYSXSCHEMA_API
    UsdAttribute CreateGridFilteringFlagsAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    // --------------------------------------------------------------------- //
    /// Number of smoothing passes applied to the generated isosurface triangle mesh's normals.
    /// Range: [0, inf)
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `int physxIsosurface:numMeshNormalSmoothingPasses = 0` |
    /// | C++ Type | int |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Int |
    PHYSXSCHEMA_API
