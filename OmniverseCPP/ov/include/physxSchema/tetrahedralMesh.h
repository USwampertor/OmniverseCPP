...
//
#ifndef PHYSXSCHEMA_GENERATED_TETRAHEDRALMESH_H
#define PHYSXSCHEMA_GENERATED_TETRAHEDRALMESH_H
...
public:
    ...
    /// an invalid \p prim
    explicit PhysxSchemaTetrahedralMesh(const UsdPrim& prim=UsdPrim())
    ...
    explicit PhysxSchemaTetrahedralMesh(const UsdSchemaBase& schemaObj)
    ...
    /// Return a PhysxSchemaTetrahedralMesh holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// PhysxSchemaTetrahedralMesh(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    PHYSXSCHEMA_API
    static PhysxSchemaTetrahedralMesh
