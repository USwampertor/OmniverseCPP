...
class BASICEXTERNALOBJECT_API DynamicLibrary {
public:
  ...
  int
	load() {
		auto newData = static_cast<void*>(LoadLibraryA(m_name.c_str()));

    if (!newData) {
      uint64_t err = GetLastError();
      g_error = "Could not find dll at given path: " + m_name + "\nError code: " + std::to_string(err);
      return -1;
    }
    if (m_data == newData) {
  ...
  void*
    ...
    auto t_api = GetProcAddress(static_cast<HINSTANCE>(m_data), symbol.c_str());
    if (!t_api) {
      return nullptr;
    }
    return t_api;
...
extern "C" BASICEXTERNALOBJECT_API long jsInit(TaggedData * argv, 
																							 long argc, 
																							 TaggedData * retval) {
  g_DLL = new DynamicLibrary("D:\\Projects\\OmniverseCPP\\OmniverseCPP\\bin\\Debug\\ovCLibraryd.dll");
	retval->type = kTypeString;
	string resultString = "Result ended with value: ";
	int result = g_DLL->load();
	resultString.append(std::to_string(result));
  if ( result == -1) {
		resultString.append(" Error initializing value \n");
		resultString.append(g_error);
    // g_DLL->loadSymbol("jsInitialize");
  }
	else if (result == 1) {
		typedef void(*VOIDInit)(bool, int);
		auto initFunction = (VOIDInit)g_DLL->loadSymbol("jsInitialize");
		initFunction(true, 2);
	}
	retval->data.string = getNewBuffer(resultString);
	return kESErrOK;
}
...
{ 
	return "makeArray,getAverage,appendString_s,myScript_a,acceptBoolean_b,jsInit";
}
...
{
  g_DLL->loadSymbol("jsShutdown");
  
  if (g_DLL) { g_DLL->unload(); }
}